...
def parse(item: dict[str, Any]) -> list[sources.RawPosting]:
    ...
    for comment in item.get("children", []):
        ...
        if not text_html:  # deleted/dead comments have no text
            continue
        text = sources.html_to_text(text_html)
        ...
        salary_min, salary_max = sources.parse_salary_range(first_line)
        postings.append(
            sources.RawPosting(
                ...
                description=text,
                posted_at=sources.parse_datetime(comment.get("created_at")),
...
class HackerNewsWhoIsHiringSource(sources.SourceAdapter):
    kind = "hn"
    default_name = "HN Who is hiring?"
